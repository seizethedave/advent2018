...
class Guard(object):
    ...
    @property
    def sleepiest_minute_with_count(self):
        if not self.sleep_counts:
            return (0, 0)

        return self.sleep_counts.most_common(1)[0]
...
def go():
    ...
    best = max(guards.itervalues(), key=lambda g: g.sleepiest_minute_with_count[1])
    print best.id * best.sleepiest_minute_with_count[0]
